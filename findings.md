...
## 2026-09-04：按体验多样性规范重新规划当前地图

- 当前地图已完成房间优先 CQB 重构：12 个房间锚点、10 个错位门洞、29 段不可破坏房间墙、14 个室内掩体和左右循环路线均已有结构与可达性回归。
- 上一轮地图验收集中在房间闭合度、门洞开放、候选点可达、出生安全和视觉密度，尚未记录每个房间的职责、主要决策、解法族、奖励成本或任务激活后的变化。
- 新的 `EXPERIENCE-VARIETY-STANDARD.md` 已强制要求遭遇矩阵、至少 6 种模板、每个战斗遭遇至少两个有效解法族、任意主路不连续三个同模板遭遇，以及五种子可玩审计。
- 本轮先重新规划而不直接修改场景；规划必须以现有 MVP 边界为前提，只使用手枪警卫、既有四种武器、掩体、可破坏墙、燃烧、爆炸、任务警报和撤离，不擅自加入刷怪、倒计时或新敌人。
- 当前未提交工作区已有体验多样性规范、ADR-081、GDD 与设计索引改动，地图规划必须保留并引用这些改动。
- `cqb_room_layout_3d.tscn` 的 12 个锚点实际分为北排 5 房、中排 5 房、南排 2 房；南排中央同时包含出生/撤离庭院，左右外侧通过 `DoorWestLoop` 与 `DoorEastLoop` 形成循环。
- 现有固定微观掩体为北排 6 个、中排 4 个、南排 4 个，分布基本对称；这种结构适合承载不同遭遇，但当前节点没有房间职责或遭遇模板标签。
- 现有门洞是北门槛 4 个、南门槛 4 个、两侧循环门 2 个；门洞错位已经能切断长射线，重规划应尽量复用而非推倒宏观拓扑。
- 主场景另有 23 段可破坏墙及石油桶/汽油桶连锁，固定环境物与房间锚点分属两个场景；规划必须建立明确映射，避免继续以全图散点理解内容。
- 16 个警卫候选当前按坐标均匀铺在北排 6 个、中排 6 个、南端/返程区 4 个；生成器从全池独立洗牌取 8–12 个，无法保证单局的遭遇模板组合或避免相邻房间同质化。
- 四个任务候选全部集中在最北侧左右两端，实质上只有“西北任务”与“东北任务”两种宏观目标方向，适合规划为左右镜像但风险结构不同的两套去程/返程节奏。
- 10 个武器候选主要集中北排，仅两个位于南排；4 件武器类型与候选位置分别独立洗牌，可能出现奖励与其适用遭遇无关，也无法保证先获得武器再遇到展示其价值的房间。
- 8 个医疗候选横跨北排、南侧外环和出生后方；生成器独立选 2 个，当前没有高风险高收益或路线取舍约束。
- 当前内容生成器只对四类候选变换分别洗牌，没有房间、风险、模板或关联环境元数据；落实规范需要从“散点抽取”升级为“遭遇组合抽取”，但可先保留种子确定性和既有数量预算。
- 固定环境的显著非对称点只有西南房间的石油桶→木墙→汽油桶连锁；中央中房有 4 段木墙屏风，东西中房、两侧南环和北内房各有局部三段木墙，具备重新分配不同模板的基础。
- 上一轮俯视截图不再存在于 `/tmp`，但仓库保留 `tests/cqb_map_visual_probe.gd`；它会隐藏 HUD、玩家和动态警卫，以 55 米高、44 米正交相机输出 1280×720 全图，可在规划完成后复用于布局视觉核对。
- 2026-09-04 重新生成并查看 `/tmp/gogogo-cqb-room-layout.png`：当前结构清晰且可达，但东西两侧高度镜像，北排多个房间重复使用“单箱/单沙袋＋水平三段木墙”，容易产生外形变化而解法相同的问题。
- 俯视图显示中央出生庭院是明确安全地标，左右各有南侧外环可回到中排；重新规划不需要推倒 29 段砖墙宏观骨架，应主要调整房间职责、固定可破坏物、候选点组合和警卫朝向。
- 西南连锁房的石油桶→木墙→汽油桶在视觉上非常突出，适合作为“环境连锁教学/高风险捷径”；东南当前结构与之近似但缺少独特问题，适合规划为“补给争夺/返程储备”形成左右非对称。
- 北排是任务目标所在的终局带，但目前五个房间视觉和资源密度接近；应把两个外侧任务室分别设计为不同的终局问题，并让三个北内房承担掩体对峙、目标排序与范围武器展示，避免到达任务前连续清同类房间。
- 除 12 个房间锚点外，出生庭院、西南外环和东南外环也需要作为独立规划区域进入遭遇矩阵，否则返程路线与喘息节拍无法被规范覆盖。
- 重规划最终采用 15 个区域：S0 出生/撤离、SW0/SE0 两侧外环、S1/S2 南侧房、M1–M5 中排、N1–N5 北排；其中 S0 为安全区，其余区域各自具有主要问题、至少两个解法族和激活后作用。
- 三条路线主题冻结为：西线“高波动、可破坏、环境连锁”，中线“信息控制、目标排序、最短但易汇合”，东线“稳定、掩体、补给、以距离换安全”。
- 西北与东北任务房不再只是坐标镜像：N1 规划为软屏风爆破/侧入终局，N5 规划为硬掩体交叉火力/双入口终局。
- 内容生成顺序冻结为“任务侧 → 遭遇组合 → 警卫槽 → 适配武器 → 风险医疗 → 规则验证”，同时保留 8–12 警卫、四件特殊武器、两个医疗和同种子确定性。
- 当前规划在设计层覆盖 7 类模板并满足路线交替，但场景尚未按规划调整，生成器仍是散点洗牌，五种子可玩审计也未执行；因此地图不能标记为完全符合规范。
